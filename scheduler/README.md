...
- ✅ AuditLog：建立 / 更新 / 刪除 Event Type 與特定日覆寫都寫稽核紀錄。

### M1 打磨（已加入）

- ✅ **公開頁月曆 UI**：右欄改為可翻月的月曆，只有「有可預約時段」的日期可點；選日後依上午 / 下午分組顯示時段。新增端點 `GET /api/event-types/:slug/availability-days?month=YYYY-MM&tz=`。
- ✅ **管理後台 CRUD 強化**：Event Type 可編輯（`/admin/event-types/:slug`）、停用 / 啟用、刪除（已有預約則擋下，請改停用）。端點 `GET/PATCH/DELETE /api/admin/event-types/:slug`。
- ✅ **DateOverride 管理介面**：`/admin/interviewers` 可為面試官設定休假（封鎖整天）或臨時時段（自訂 / 沿用每週）。端點 `POST/DELETE /api/admin/interviewers/:id/date-overrides`。
- ✅ **表單與驗收細節**：建立關卡時 slug 由標題自動帶入、時段上午 / 下午分組、空 / 載入狀態、錯誤訊息中文化。
...
- 公開預約頁（月曆 + 時段）：<http://localhost:3000/senior-recruiter-jira690>
- 管理後台（Event Type CRUD）：<http://localhost:3000/admin>
- 面試官與特定日設定（DateOverride）：<http://localhost:3000/admin/interviewers>
